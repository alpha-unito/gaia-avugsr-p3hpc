...
inline double maxCommMultiBlock_double(const double* __restrict__ gArr, const L arraySize){
    ...
    #ifndef __ACPP__
        return  std::fabs(*std::max_element(POL, gArr, gArr+arraySize, 
        ...
        double red=std::transform_reduce(
                    POL,
                    gArr, gArr+arraySize,
                    -std::numeric_limits<double>::infinity(), // Start reduction with the first iterator
                    [=](double a, double b) {
                        return std::max(a,b); // Custom reduction: select the larger element
                    },
                    [=](double it) {
                        return std::fabs(it); // No transformation, we are just passing the iterator itself
                    }
                );
        return std::fabs(red);
    #endif
                                ...
                                const short nAttParAxis)
{
    ...
    std::for_each(POL, start, end,[=](const auto& ix){
        ...
        const long tmp=static_cast<long>(ix*nAttP);
        #pragma unroll
        for(short inpax = 0;inpax<nAttParAxis; ++inpax)
            sum += systemMatrix_dev[tmp + inpax ] * vVect_dev[jstartAtt + inpax];
        #pragma unroll
        for(short inpax = 0;inpax<nAttParAxis;inpax++)
            sum+=  systemMatrix_dev[tmp+nAttParAxis+inpax] * vVect_dev[jstartAtt+nDegFreedomAtt+inpax];
        #pragma unroll
        for(short inpax = 0;inpax<nAttParAxis;inpax++)
            sum+= systemMatrix_dev[tmp + nAttParAxis+nAttParAxis +inpax] * vVect_dev[jstartAtt+nDegFreedomAtt+nDegFreedomAtt+inpax];
...
void lsqr(
    ...
    double t{ZERO};
        ...
        // hipMemAdvise(vVect, sizeof(double)*nunkSplit, hipMemAdviseSetCoarseGrain, deviceNum);
        // hipMemAdvise(knownTerms, sizeof(double)*nElemKnownTerms, hipMemAdviseSetCoarseGrain, deviceNum);
    
    #endif

    // std::vector<double> knownTerms_loc(nEqExtConstr+nEqBarConstr+nOfInstrConstr);
    ...
    while (1) {
        ...
        // std::copy_n(POL,&knownTerms[mapNoss],nEqExtConstr+nEqBarConstr+nOfInstrConstr,knownTerms_loc.data());
        std::copy_n(POL,&knownTerms[mapNoss],nEqExtConstr+nEqBarConstr+nOfInstrConstr,knownTerms_loc);
        //------------------------------------------------------------------------------------------------
        starttime=MPI_Wtime();
            // MPI_Allreduce(MPI_IN_PLACE,knownTerms_loc.data(),nEqExtConstr+nEqBarConstr+nOfInstrConstr,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
        ...
        // std::copy_n(POL,knownTerms_loc.data(),nEqExtConstr+nEqBarConstr+nOfInstrConstr,&knownTerms[mapNoss]);
        std::transform(POL,
                        kAuxcopy,
                        kAuxcopy+nEqExtConstr+nEqBarConstr+nOfInstrConstr,
                        knownTerms_loc,
